...
| S15 | The synthetic token literal from the evidence-reader scenario appears nowhere under `out/demo`; its SHA-256 does. | `T=$(grep -o 'ghp_[A-Za-z0-9]*' packages/fixtures/scenarios/evidence-reader.yaml \| head -1); (grep -r "$T" out/demo \|\| true) \| wc -l; grep -rl "$(printf %s "$T" \| shasum -a 256 \| cut -c1-64)" out/demo \| wc -l` | first `0`; second ≥ 1 |
