...
> Lineage: this is the third build ("charlie"). The `colophon-alpha` and `colophon-beta` repositories were the two earlier shapes (Agent-Card era) and are superseded by this one.
...
- Bun ≥ 1.4 (`bun.lock` is lockfile version 2, which older Bun cannot parse under `--frozen-lockfile`), TypeScript `strict`. OPA ≥ 1.0 with `import rego.v1`. Cosign 3.x. `bun`/`bunx` only.
...
- No credential value, synthetic or real, appears in any trace, evidence item, report, or bundle. Arguments are stored as their SHA-256 plus a redacted view; redaction commits credential-shaped substrings, not only whole values; a packet whose COL-10 check fails is refused before signing.
- All names, repos, tokens, and people in fixtures are synthetic; domains use `.example`. The one live capture (S40) is a real Gateway session with the account id redacted and the principal name generalized.
- Paths inside a packet are relative to the directory it was built from. A packet is portable evidence; it never carries the build machine's absolute path.
...
colophon/
  ...
  packages/
    ...
    adapters/    colophon-gate, claude-hook (fixture), aws-config (CloudTrail/IAM fixture), agentcore-dogwood (AgentCore+Dogwood AuthorizeAction + APPLICATION_LOGS capture; no AWS SDK)
    hook/        Colophon as a Claude Code PreToolUse hook (`colophon hook`) + session-end `colophon seal`: the gate's verdict path over a second transport
...
| S41 | Redaction is substring-based with the same credential pattern COL-10 scans for: a token embedded inside a `Bash` command in a claude-hook event is committed (`sha256:<hex>` in place) and the command keeps its shape; the literal never reaches the sealed trace. The token is built at runtime so no committed literal is credential-shaped. | `T="gh"; T="${T}p_$(printf 'Q%.0s' $(seq 1 36))"; jq -nc --arg t "$T" '{hook_event_name:"PreToolUse",session_id:"s41",cwd:"/work",tool_name:"Bash",tool_input:{command:("curl -H \"Authorization: Bearer "+$t+"\" https://api.example")},ts:"2026-09-15T00:00:00Z",hook_response:{hookSpecificOutput:{permissionDecision:"allow",permissionDecisionReason:"[CC-HOOK-CWD] ok"}}}' > out/probe/s41.jsonl; $CLI normalize claude-hook out/probe/s41.jsonl --out out/probe/s41 >/dev/null; (grep -c "$T" out/probe/s41/trace/*.jsonl \|\| true); grep -c "sha256:$(printf %s "$T" \| shasum -a 256 \| cut -c1-64)" out/probe/s41/trace/*.jsonl; jq -r '.args_redacted.command' out/probe/s41/trace/*.jsonl \| grep -c 'curl -H "Authorization: Bearer sha256:'` | `0`; ≥ 1; `1` |
| S42 | A packet whose COL-10 (no credential values) is not-satisfied is refused before signing: `buildPacket` throws naming `COL-10`, writes no bundle, and removes its staged copy. Enforced in `packages/cli/packet.ts`, exercised by `tests/packet-refuse.test.ts` without cosign (the refusal precedes signing). | `bun test tests/packet-refuse.test.ts 2>&1 \| tail -3; grep -c 'refusing to sign' packages/cli/packet.ts` | 0 fail; ≥ 1 |
...
| S43 | No file in any demo bundle carries the build machine's absolute path: narrative, session.json, evidence items, and the OSCAL AR name fixture and bundle paths relative to the repo root, so the packet reads the same on every machine. | `(grep -rlF "$PWD" $OUT/*/bundle \|\| true) \| wc -l; (grep -rlF "$HOME" $OUT/*/bundle \|\| true) \| wc -l; grep -c 'bundle verify out/demo/' $OUT/agentcore-dogwood-live/bundle/report/narrative.md` | `0`; `0`; ≥ 1 |
...
| S40 | The agentcore-dogwood adapter ingests captured AgentCore Gateway APPLICATION_LOGS JSONL: joins `Started processing request` to `Policy evaluation completed`/`denied` on `request_id`; session_id comes from sidecar metadata (not the log body); the live ENFORCE RoE fixture yields five decisions allow/deny/allow/allow/deny with `AGENTCORE-DEFAULT-DENY` on empty determiningPolicies; sealed packet + Finding export work offline; missing sidecar without `--session` exits 1; no AWS SDK. The bound Record names `pep.tool_name_prefix` (`StatusTarget___`) so COL-05 re-evaluates the declared tool names and reports satisfied as declaration consistency; the narrative's refusal section quotes the Gateway's reason and never renders `request_id` or the principal as a bound. | `$CLI normalize agentcore-dogwood packages/fixtures/agentcore-dogwood/live-roe-7461903f.jsonl --out out/probe/ac-live && $CLI trace verify out/probe/ac-live/trace/*.jsonl && $CLI export finding --from out/probe/ac-live --out out/probe/ac-live-findings && $CLI export finding validate out/probe/ac-live-findings/*.finding.json && $CLI bundle verify $OUT/agentcore-dogwood-live/bundle --pubkey $OUT/keys/cosign.pub; grep -c 'COL-05 .*\*\*satisfied\*\*' $OUT/agentcore-dogwood-live/bundle/report/narrative.md; grep -c 'request_id.*fell outside' $OUT/agentcore-dogwood-live/bundle/report/narrative.md` | exits 0; five decisions in the allow/deny/allow/allow/deny sequence; session `7461903f-e0c0-41ce-844b-87d43dcb1a23`; Finding `ai_agent_session`; bundle verified; copy without sidecar exits 1; no AWS SDK; COL-05 satisfied `1`; request_id-as-bound `0` |

### F10 · Colophon for Claude Code (hook + seal)

| # | Claim | Probe | Pass |
|---|---|---|---|
| S44 | `$CLI hook --record --pubkey --trace-dir` reads one Claude Code PreToolUse event on stdin, binds the Record on every call, decides through `gate.rego`, appends one chained Decision to `<trace-dir>/<session_id>.jsonl`, and prints `{hookSpecificOutput:{permissionDecision}}` (allow / deny / ask for escalate). Over the events fixture: Read, Edit, Write inside the sandbox allow; Bash asks (`requires_approval`); a Write outside cwd and a `src/../.env` write deny `COL-GATE-SANDBOX`; a fetch to an undeclared host denies `COL-GATE-DESTINATION`; an undeclared MCP tool denies `COL-GATE-UNKNOWN-TOOL`. The first call writes the fail-closed self-test. | `rm -rf out/probe/hook; while IFS= read -r l; do printf '%s' "$l" \| $CLI hook --record $OUT/records/claude-coder.record.json --pubkey $OUT/keys/cosign.pub --trace-dir out/probe/hook 2>/dev/null \| jq -r '.hookSpecificOutput.permissionDecision'; done < packages/fixtures/claude-hook/events.jsonl \| paste -sd, -; jq -r '.rule_ids[0]' out/probe/hook/cc-live-0001.jsonl \| paste -sd, -; $CLI trace verify out/probe/hook/cc-live-0001.jsonl; ls out/probe/hook/cc-live-0001.selftest.json` | `allow,allow,allow,ask,deny,deny,deny,deny`; `COL-GATE-ALLOW,COL-GATE-ALLOW,COL-GATE-ALLOW,COL-GATE-APPROVAL,COL-GATE-SANDBOX,COL-GATE-DESTINATION,COL-GATE-SANDBOX,COL-GATE-UNKNOWN-TOOL`; `sealed: yes`; self-test file listed |
| S45 | The hook fails closed: malformed stdin, a Record whose signature file is missing, and an OPA failure (`COLOPHON_GATE_POLICY` pointing at a nonexistent file) each answer `deny` under `COL-GATE-OPA-ERROR`, exit 0, and the deny is appended to the trace whenever a session id was present. | `echo 'not json' \| $CLI hook --record $OUT/records/claude-coder.record.json --pubkey $OUT/keys/cosign.pub --trace-dir out/probe/hook-fc 2>/dev/null \| jq -r '.hookSpecificOutput.permissionDecision'; cp $OUT/records/claude-coder.record.json out/probe/unsigned-cc.record.json; head -1 packages/fixtures/claude-hook/events.jsonl \| $CLI hook --record out/probe/unsigned-cc.record.json --pubkey $OUT/keys/cosign.pub --trace-dir out/probe/hook-fc 2>/dev/null \| jq -r '.hookSpecificOutput.permissionDecision'; head -1 packages/fixtures/claude-hook/events.jsonl \| COLOPHON_GATE_POLICY=/nonexistent/gate.rego $CLI hook --record $OUT/records/claude-coder.record.json --pubkey $OUT/keys/cosign.pub --trace-dir out/probe/hook-fc2 2>/dev/null \| jq -r '.hookSpecificOutput.permissionDecisionReason' \| cut -c1-21; jq -r '.effect+":"+.rule_ids[0]' out/probe/hook-fc/cc-live-0001.jsonl out/probe/hook-fc2/cc-live-0001.jsonl` | `deny`; `deny`; `[COL-GATE-OPA-ERROR]`; two lines `deny:COL-GATE-OPA-ERROR` |
| S46 | `$CLI seal` turns a hook session's trace into an assessed, signed, verified packet (source `colophon-hook`), prints `SIGNATURE:` only after verify, and a session that had refusals reads 8/8; the demo's `claude-coder` packet was produced by the real hook path and verifies. | `$CLI seal --session cc-live-0001 --trace-dir out/probe/hook --record $OUT/records/claude-coder.record.json --pubkey $OUT/keys/cosign.pub --key $OUT/keys/cosign.key --password colophon-demo --out out/probe/sealed --name cc-probe \| grep -E '^packet\|^SIGNATURE'; $CLI bundle verify out/probe/sealed/cc-probe/bundle --pubkey $OUT/keys/cosign.pub \| tail -1; $CLI bundle verify $OUT/claude-coder/bundle --pubkey $OUT/keys/cosign.pub \| tail -1; grep -c '\*\*satisfied\*\*' $OUT/claude-coder/bundle/report/narrative.md; grep -c 'PreToolUse hook' $OUT/claude-coder/bundle/report/narrative.md; grep -c 'sealSession' packages/cli/demo.ts` | `controls 8/8`; `SIGNATURE:`; two `verified:` lines; `8`; ≥ 1; ≥ 1 |
| S47 | `$CLI hook settings` prints the documented `.claude/settings.json` fragment (`hooks.PreToolUse[].matcher` empty = every tool, `type: command`, 30 s timeout, absolute command); tool mapping, path relativization, `defaults.data_class` provenance, and replay parity with the `claude-hook` adapter are unit-tested. | `$CLI hook settings --record $OUT/records/claude-coder.record.json --pubkey $OUT/keys/cosign.pub --trace-dir out/hook > out/probe/hook-settings.json; jq -r '.hooks.PreToolUse[0].matcher, .hooks.PreToolUse[0].hooks[0].type, .hooks.PreToolUse[0].hooks[0].timeout, (.hooks.PreToolUse[0].hooks[0].command\|split(" ")\|.[2])' out/probe/hook-settings.json; bun test tests/hook.test.ts 2>&1 \| tail -3` | empty line; `command`; `30`; `hook`; 0 fail |
...
| S34 | STATUS.md has one row per claim S1–S47 and A1–A3; every `done` row names the probe run and the session date. | `grep -cE '^\| (S[0-9]+\|A[0-9]+) ' STATUS.md` | `50` |
