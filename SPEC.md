...
| S40 | The agentcore-dogwood adapter ingests captured AgentCore Gateway APPLICATION_LOGS JSONL: joins `Started processing request` to `Policy evaluation completed`/`denied` on `request_id`; session_id comes from sidecar metadata (not the log body); the live ENFORCE RoE fixture yields five decisions allow/deny/allow/allow/deny with `AGENTCORE-DEFAULT-DENY` on empty determiningPolicies; sealed packet + Finding export work offline; missing sidecar without `--session` exits 1; no AWS SDK. | `$CLI normalize agentcore-dogwood packages/fixtures/agentcore-dogwood/live-roe-7461903f.jsonl --out out/probe/ac-live && $CLI trace verify out/probe/ac-live/trace/*.jsonl && $CLI export finding --from out/probe/ac-live --out out/probe/ac-live-findings && $CLI export finding validate out/probe/ac-live-findings/*.finding.json && $CLI bundle verify $OUT/agentcore-dogwood-live/bundle --pubkey $OUT/keys/cosign.pub` | exits 0; five decisions in the allow/deny/allow/allow/deny sequence; session `7461903f-e0c0-41ce-844b-87d43dcb1a23`; Finding `ai_agent_session`; bundle verified; copy without sidecar exits 1; no AWS SDK |
...
| S34 | STATUS.md has one row per claim S1–S40 and A1–A3; every `done` row names the probe run and the session date. | `grep -cE '^\| (S[0-9]+\|A[0-9]+) ' STATUS.md` | `43` |
