...
const P: Probe[] = [
  ...
  { id: "S40", cmd: `$CLI normalize agentcore-dogwood packages/fixtures/agentcore-dogwood/live-roe-7461903f.jsonl --out out/probe/ac-live && $CLI trace verify out/probe/ac-live/trace/*.jsonl && jq -r '.effect+":"+.rule_ids[0]+":"+.tool' out/probe/ac-live/trace/*.jsonl && jq -r '.session_id' out/probe/ac-live/trace/*.jsonl | sort -u && $CLI export finding --from out/probe/ac-live --out out/probe/ac-live-findings && $CLI export finding validate out/probe/ac-live-findings/*.finding.json && $CLI bundle verify $OUT/agentcore-dogwood-live/bundle --pubkey $OUT/keys/cosign.pub && $CLI export finding validate $OUT/agentcore-dogwood-live/findings/*.finding.json; echo "sid=$(jq -r '.resource.id' $OUT/agentcore-dogwood-live/findings/*.finding.json)"; mkdir -p out/probe/ac-nometasrc && cp packages/fixtures/agentcore-dogwood/live-roe-7461903f.jsonl out/probe/ac-nometasrc/; set +e; $CLI normalize agentcore-dogwood out/probe/ac-nometasrc/live-roe-7461903f.jsonl --out out/probe/ac-nometa; echo "nometa=$?"; set -e; $CLI normalize agentcore-dogwood out/probe/ac-nometasrc/live-roe-7461903f.jsonl --session 7461903f-e0c0-41ce-844b-87d43dcb1a23 --out out/probe/ac-flag && jq -r '.session_id' out/probe/ac-flag/trace/*.jsonl | sort -u; echo "deps=$(jq -r '(.dependencies + .devDependencies)|keys[]' package.json | grep -cE 'aws-sdk|bedrock' || true)"; echo "live=$(grep -rnE 'LiveAws|@aws-sdk' packages/adapters/agentcore-dogwood --include='*.ts' | wc -l | tr -d ' ')"`, pass: (r) => {
  ...
  }, show: (r) => lines(r.out).filter((l) => /^(allow|deny):|nometa=|deps=|live=|sid=/.test(l)).join(" ") },
  ...
  { id: "S34", cmd: `grep -cE '^\\| (S[0-9]+|A[0-9]+) ' STATUS.md`, pass: (r) => num(r.out) === 43 },
